...
from tools.terminal_tool_lifecycle import is_persistent_env  # Backward-compatible test/patch seam.
...
def _provider_preferences_for_agent(agent) -> Dict[str, Any]:
    ...
    merged["preferred_min_throughput"] = _validated_openrouter_perf_value(
        getattr(agent, "provider_preferred_min_throughput", None),
        label="preferred_min_throughput",
    )
    merged["preferred_max_latency"] = _validated_openrouter_perf_value(
        getattr(agent, "provider_preferred_max_latency", None),

...
def _batch_admission(tool_call, execution_cwd: Optional[Path]) -> tuple[str, List[Path], bool] | None:
    ...
    if name in _PATH_SCOPED_TOOLS:
        # Named execution targets resolve cwd/path identity lazily in the tool
        # layer, so this lexical planner cannot compare scopes safely: treat
        # target-selected calls (or any call when a named default exists) as
        # conservative barriers (#2 named-execution-targets).
        target_selected = (

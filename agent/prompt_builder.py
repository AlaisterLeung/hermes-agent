...
# Per-process probe cache keyed by (home key, env_type, target, config identity): a
# backend/target switch rebuilds it; home key because one process multiplexes profiles.
...
def _probe_remote_backend(
    ...
    cache_key = (hermes_home_key(), env_type, target_name, config_identity)
    cached = _BACKEND_PROBE_CACHE.get(cache_key)
    if cached is not None:
        return cached or None

    try:
        # Local import: tools/ imports are heavy and only matter for a non-local
        # backend. Env factory lives in tools.terminal_tool_backends (tests patch it there).
    ...
    try:
        ...
        # Build like tools/terminal_tool.py does for a live command: select the
        # backend image, assemble config from the env dict (`_create_environment`).
        if env_type == "docker":
        ...
        # Without teardown the probe leaves a second idle sandbox
        # (task_id="prompt-backend-probe") alive for the whole process lifetime;
        # probe_only (isolated socket, no sync/snapshot) makes cleanup safe for ssh.
        if env is not None:
            try:
                from tools.terminal_tool_lifecycle import _cleanup_env
...
def build_environment_hints() -> str:
    ...
    # avoids forking the identity slot (SOUL.md); read once for cache safety.
    # `HERMES_ENVIRONMENT_HINT` wins over config.yaml ``agent.environment_hint``.

...
For SSH targets, `file_sync: false` skips the automatic `~/.hermes` mirror (the remote directory setup plus the credential/skills/cache upload and the teardown pull-back). This is intended for remote hosts that are real, self-managed machines rather than ephemeral sandboxes — it avoids redundant or wasteful syncs and avoids creating `~/.hermes` on hosts that don't need it. The default is `true`, preserving the existing sandbox-provisioning behavior.

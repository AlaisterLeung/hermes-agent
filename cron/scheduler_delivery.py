...
def _open_continuable_cron_thread(job: dict, adapter, chat_id: str, loop) -> Optional[str]:
    ...
    if not callable(create_thread) or loop is None:
        return None
    thread_name = f"[Cron] {_cron_display_name(job)}"
    try:
...
def _seed_cron_thread_session(
    ...
    (``_THREAD_REPLY_CHAT_TYPE``; Matrix room threads keep the ROOM's ``group`` key). Matrix
    DM-ness is re-resolved live via the adapter's ``_is_dm_room`` — job metadata and the live
    classification can disagree, and the seed must match the arm the REPLY will take;
    best-effort — a successful delivery is never failed by the probe."""
    text = (mirror_text or "").strip()
    if not text:
        return
    try:

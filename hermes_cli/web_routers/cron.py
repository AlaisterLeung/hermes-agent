...
_forward_cron_fire_to_gateway_sync = late("_forward_cron_fire_to_gateway_sync", "hermes_cli.web_server_cron")
...
def _trigger_cron_job_sync(job_id: str, profile: Optional[str] = None):
    ...
    Execution belongs to the GATEWAY process (live adapters + session context):
    the fire is forwarded to the gateway api_server, which claims via the store
    CAS (a concurrent ticker or retry cannot double-run) and runs with live
    adapters. Paused jobs pass ``force`` so the claim resumes-and-fires
    atomically; an unreachable gateway surfaces as 503 rather than a silent
    standalone-path delivery from this process.
    """
    selected = _job_profile(job_id, profile)

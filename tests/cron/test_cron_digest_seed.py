...
Contract: when mirroring is enabled (per-job ``attach_to_session`` or the
global ``cron.mirror_delivery`` flag), the seed appended to the continuation
session carries a bounded digest of the run's own transcript — not just the
final response; with mirroring off nothing changes. The digest is pure
enrichment — every failure path falls back to the plain text.
"""
...
class TestDeliveryPathWiring:
    ...
    recorder (same pattern as test_mirror_origin_fallback.py): the seed
    carries the digest when mirroring is on, the plain text when off."""
...
def cron_session_db(tmp_path, monkeypatch):
    ...
    def _insert(session_id, role, content=None, tool_name=None, tool_calls=None,
        ...
        # Commit: the digest reads via its own connection; uncommitted

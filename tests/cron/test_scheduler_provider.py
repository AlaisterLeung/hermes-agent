...
def test_desktop_ticker_forwards_due_jobs_to_gateway_then_stops():
    """The desktop ticker never executes jobs locally: it forwards every due
    job to the gateway api_server's cron-fire endpoint (which owns the live
    adapters + session context) and exits once stop_event is set."""
    import httpx

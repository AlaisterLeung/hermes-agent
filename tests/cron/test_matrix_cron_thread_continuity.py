...
Before this fix an in-thread reply to a cron brief landed in a fresh empty
session: the adapter had no handoff hook, and the seed keyed a
(thread, system:cron) shape an in-thread room reply never resolves to.
...
from cron.scheduler_delivery import _open_continuable_cron_thread, _seed_cron_thread_session

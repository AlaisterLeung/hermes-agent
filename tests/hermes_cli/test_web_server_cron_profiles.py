...
async def test_trigger_cron_job_forwards_to_gateway_and_returns_refreshed_state(
    ...
    monkeypatch.setattr(
        _web_server_cron, "_forward_cron_fire_to_gateway_sync", fake_forward
    )
    monkeypatch.setattr(
        cron_jobs,
        "trigger_job",
...
def test_trigger_forward_uses_gateway_endpoint_key_and_force_body(
    ...
    monkeypatch.setattr(
        _web_server_cron, "_gateway_fire_endpoint",
    ...
    monkeypatch.setattr(_web_server_cron, "_cron_profile_home", lambda p: (p, default_home))

    result = _web_server_cron._forward_cron_fire_to_gateway_sync(
...
async def test_trigger_cron_job_reports_unreachable_gateway_as_unavailable(
    ...
    persisted = _web_server_cron._call_cron_for_profile("worker_alpha", "get_job", job["id"])

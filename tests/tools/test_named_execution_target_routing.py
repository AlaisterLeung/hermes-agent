...
from tools.file_tools_read_tracking import notify_other_tool_call as notify_other_tool_call_tracking
from tools.file_tools_read_tracking import reset_file_dedup as reset_file_dedup_tracking
from tools.terminal_tool_lifecycle import cleanup_vm as cleanup_vm_lifecycle
from tools.terminal_tool_sudo import (
    _get_cached_sudo_password as _sudo_get_cached,
    _invalidate_cached_sudo_on_auth_failure as _sudo_invalidate_on_auth_failure,
    _reset_cached_sudo_passwords as _sudo_reset_cached,
    _set_cached_sudo_password as _sudo_set_cached,
    _sudo_nopasswd_works as _sudo_nopasswd,
)
...
def test_execute_code_inherits_target_and_rejects_nested_override(
    ...
    monkeypatch.setattr(
        ...
        expected_target_scope=None, expected_target_config=None, reset=False: (
            forwarded.update(
...
def test_sudo_cache_and_nopasswd_probe_are_target_scoped(
    ...
    _sudo_set_cached("local-secret", "local", "local")
    assert _sudo_get_cached("local", "local") == "local-secret"
    assert _sudo_get_cached("devbox", "ssh") == ""
    ...
    with terminal_mod._scoped_sudo_execution("devbox", "ssh"):
        assert _sudo_nopasswd() is False
    assert calls == []

    with terminal_mod._scoped_sudo_execution("local", "local"):
        assert _sudo_nopasswd() is True
    assert len(calls) == 1
    ...
    with terminal_mod._scoped_sudo_execution("devbox", "ssh", named=True):
        _sudo_set_cached("cached-target")
    ...
    with terminal_mod._scoped_sudo_execution(
        ...
        _sudo_set_cached("stale-secret")
    assert _sudo_invalidate_on_auth_failure(
        "sudo id", "sudo: authentication failed", "devbox", "ssh", "config-v2",
    ) is True
    assert _sudo_get_cached(

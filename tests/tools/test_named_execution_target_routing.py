...
def test_workspace_override_applies_only_to_default_named_target(
    ...
    # ACP/TUI/gateway register the host workspace as a task override: seed the
    # configured default target, don't replace an explicit remote target's cwd.
    terminal_mod.register_task_env_overrides(
...
def test_cleanup_without_target_removes_all_task_scopes_and_explicit_removes_one(
    ...
    # Unregistered delegate ids collapse to the parent's "default" scope, but
    # closing a delegate must not tear down the parent's shared environments.
...
def test_execute_code_routes_real_nested_file_call_to_selected_local_target(
    ...
    # Real merged configs carry defaults for unrelated backends: freezing the
    # approved target must keep them inherited, not local-target fields.

...
def test_named_target_disables_ssh_file_sync():
    """Named execution targets must not mirror ``~/.hermes`` to the remote
    host; the legacy flat ssh path keeps the historical sandbox mirror.

    The suppression is derived from the resolution (``resolution.named``) —
    there is deliberately no per-target ``file_sync`` config key to toggle.
    """
    ...
    named = ExecutionTargetResolution(
        target="devbox",
    ...
    _, ssh_config, _ = _build_environment_constructor_configs(
        canonical, legacy, base_task_id="t1",
    )
    assert ssh_config is not None
    assert ssh_config["file_sync"] is True

...
def test_profile_local_mcp_tool_is_visible_in_slash_worker(tmp_path):
    ...
    (profile_home / "config.yaml").write_text(
        yaml.safe_dump(
            {
                # The worker waits at most `mcp_discovery_timeout` (production default
                # 1.5s) for discovery before it serves /tools. A loaded runner's cold
                # MCP handshake can exceed that, after which the worker answers from an
                # incomplete registry — give the probe a bound that is never the race.
    ...
    try:
        ...
        proc.stdin.flush()
        try:
            # The worker sends its first response only after its bounded discovery wait
            # (mcp_discovery_timeout above); leave ample slack over that bound.
            line = output.get(timeout=90)
        except queue.Empty:
            pytest.fail("slash worker produced no /tools response within 90 seconds")

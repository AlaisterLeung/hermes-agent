...
class GatewayTurnMixin:
    ...
    def _proxy_stream_consumer(self, source: "SessionSource", event_message_id, _thread_metadata, _run_still_current):
        ...
        if not _scfg.globally_enabled:
            return None
        from gateway.display_config import resolve_display_setting
        # Fork per-chat display overrides (PR #21): a chats entry for this chat
        # wins over the platform value, then upstream's enabled_for gate decides.
    ...
    def _run_agent_display_settings(self, source: SessionSource) -> "GatewayRunner._RunAgentDisplay":
        ...
        # Resolve the mode and its provenance together (per-chat overrides count as intent):
        # null inherits, tier off is not an operator request to disable cards. A raw
        # os.getenv here reads whichever profile's env loaded last under multiplexing
        ...
        progress_mode, _tool_progress_explicit = resolve_tool_progress(
            user_config, platform_key, get_secret("HERMES_TOOL_PROGRESS_MODE"), chat=source,
        )

...
def _has_platform_display_override(
    ...
    With ``chat`` given, a per-chat entry
    (``display.platforms.<platform>.chats.<chat_id>``) satisfies the override too.
    """
...
def _resolve_gateway_display_bool(
    ...
    Scratch-text is too noisy for threaded surfaces (Mattermost): they need an explicit per-platform
    override — a per-chat entry satisfies that opt-in too.
    """
    ...
    if (
        ...
        and not _has_platform_display_override(user_config, platform_key, setting, chat)):
        return False

    from gateway.display_config import resolve_display_setting

    value = resolve_display_setting(user_config, platform_key, setting, default, chat=chat)

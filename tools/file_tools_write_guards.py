...
def _check_sensitive_path(
    ...
    *, _resolution: Any = None,
    ...
    if resolved in _SENSITIVE_EXACT_PATHS or normalized in _SENSITIVE_EXACT_PATHS:
        return _err
    # Prevent direct edits to the Hermes config file: a malicious or prompt-injected
    # agent could silently disable exec approval (approvals.mode lives here).
...
# .cursorrules (or a project-local .hermes tree) outlives the turn and poisons
# later sessions — such writes ALWAYS require human approval (even under --yolo)
# and fail closed with none. Ported from RooCodeInc/Roo-Code (Apache-2.0);
# write_file/patch vector (#58631 covers terminal); basenames match ANY dir, case-insensitive.

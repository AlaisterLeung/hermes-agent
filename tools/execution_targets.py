...
def execution_target_config_scope(config: Mapping[str, Any]):
    ...
    try:
        ...
        # A reload during the frozen dispatch is authority for the next call; adopt
        # it only when leaving the outermost scope — nested scopes stay pinned.
        if not _frozen_config_active.get():
            with _classic_config_lock:
...
def synthesize_legacy_default_target(
    ...
    if multiplex:
        ...
        # Start from the canonical env legacy terminal execution actually uses;
        # only raw/managed authority (or conservative fallback) replaces ambient values.
...
def resolve_execution_target(
    ...
    *,
    ...
    if raw_targets is None or raw_targets == {}:
        ...
        # Legacy flat mode keeps historical TERMINAL_ENV precedence (launcher/.env
        # beats config.yaml): metadata must name the backend that will actually execute.
        backend = str(

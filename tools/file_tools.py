...
def write_file_tool(path: str, content: str, task_id: str = "default",
    ...
    try:
        ...
        with file_state.lock_path(_resolved, namespace=state_namespace):
            ...
            # Upstream feature: warn when a whole-file write mostly re-sends
            # what is already on disk. Legacy/local path only — the hint reads
            # old content through the default task file ops.
            rewrite_hint = _whole_file_rewrite_hint(
                task_id, _resolved, content,
            ) if not resolution.named else None

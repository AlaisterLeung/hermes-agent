...
class _ChildRun:
    ...
    def seed_workspace(self) -> None:
        ...
        with _quiet("Child cwd seed failed: %s"):
            ...
            # Target-aware superset of the legacy single-cwd copy: seed the
            # child's cwd record from every scope the parent owns, then keep
            # the legacy single-cwd seed for backends without target scope.

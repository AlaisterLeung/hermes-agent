...
# Raw-bytes INGEST budget — the 50MB download cap (_VISION_MAX_DOWNLOAD_BYTES),
# deliberately above the 20MB provider payload cap (_MAX_BASE64_BYTES, enforced
# post-resize at the call sites): oversized raws must reach the resizer, so
# capping raw at 20MB would reject 20-50MB photos before downscaling.
...
class ResolveContext:
    ...
    # Optional named execution target; None follows ``terminal.default_target``
    # exactly like an omitted file-tool ``target``.
...
async def resolve_image_source(
    ...
    *,
    ...
    # Confinement: under a non-local backend a path is host-readable only in a
    # media cache (after translating back to its host mount); everything else
    # is read inside the sandbox via exec-read. Backend = the target resolution.
    ...
    if host_target is not None and host_target.is_file():
        ...
        # secret-bearing files with a specific error rather than an incidental
        # magic-byte rejection; import is best-effort, a real block propagates.
        try:
    ...
    # Not a permitted host read (or host file absent): read inside the backend
    # — under a sandbox that never touches the host filesystem.
...
def _is_local_terminal_backend() -> bool:
    ...
    ``TERMINAL_ENV`` precedence; the read goes through the per-turn terminal
    scope so a routed multiplex profile sees ITS backend.
    """
...
async def _resolve_container_fallback(
    ...
    # _backend_file_ops looks up/lazily creates (issue #62825) the task env
    # under the resolved target. Blocking I/O — keep it off the event loop.
    def _read() -> Any:
...
def _finalize(
    ...
    if "image" in permitted and b"<svg" in data[:4096].lower():
        # SVG passes through — vision call sites rasterize via
        # _normalize_to_supported_image (providers ingest raster only).

...
def _action_create(a: Dict[str, Any]) -> str:
    ...
    try:
        job = create_job_with_scheduler_registration(
            ...
            # CLI-only lane: per-job budgets stay a user decision too.
            max_turns=a["max_turns"], run_budget_seconds=a["run_budget_seconds"],
...
def _update_core_fields(job: Dict[str, Any], a: Dict[str, Any], updates: Dict[str, Any]) -> Optional[str]:
    """prompt / name / deliver / skills / model pins and per-job budgets; returns an error
    string or None."""
    ...
    if a["max_turns"] is not None:
        # CLI-only lane; update_job validates, empty string clears the override.
...
Jobs run in a fresh session with no current-chat context, so prompts must be self-contained, and the agent's FINAL RESPONSE is what gets delivered — cron runs are autonomous and cannot ask questions. Jobs run on the main agent model (whatever `hermes model` is set to when they fire) unless the user pins one. Prefer updating an existing job over creating near-duplicates.""",
    "parameters": {
        ...
        "properties": {
            ...
            "action": {
                "type": "string",
                "description": "One of: create, list, update, pause, resume, remove, run. When action=create, 'prompt' is REQUIRED; 'schedule' is required unless you intend a trigger-only job (pass '' — the job then never fires on its own and runs only via action='run' or external events)."
            },
            "job_id": {
                "type": "string",

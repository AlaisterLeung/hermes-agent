...
class FileStateRegistry:
    ...
    def check_stale(self, task_id: TaskKey, resolved: str,
        ...
        if stamp is None and last_writer is None:  # net-new file / first touch
            return None
        # Remote/container state (``stat_path=False``) carries mtime=None: the
        # freshness contract there is writer-identity + timestamps only, so a
        # missing host stat must not veto the stale verdict for a path that
        # only exists on the backend.
        current_mtime = _mtime_or_none(resolved)
        if current_mtime is None:
            # If every state record for this path is mtime-less (a remote
            # container path that only exists on the backend), fall through to
            # timestamp/identity comparison instead of vetoing on the host stat.

...
def _get_or_create_env(
    ...
    with task_lock:
        ...
        if env_type == "ssh" or env_type == "local":
            # Target-aware ssh/local extras (file_sync suppression, runtime-scope
...
def _execute_remote(code: str, task_id: Optional[str], enabled_tools: Optional[List[str]],
    ...
    resolution = resolve_execution_target(target)
    # Legacy omitted/default selection routes nested calls to the one existing environment;
    # keep it byte-for-byte compatible, injecting the selector only when named targets are configured.
...
def execute_code(
    ...
    try:
        ...
        return tool_error(str(exc))
    # Legacy omitted/default selection routes nested calls to the one existing environment;
    # preserve that call shape, propagating a selector only when named targets are configured.
    inherited_target = resolution.target if resolution.named else None

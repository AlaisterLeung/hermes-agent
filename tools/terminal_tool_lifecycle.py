...
def _cleanup_inactive_envs(lifetime_seconds: int = 300):
    ...
    # Phase 1: collect stale entries and remove them from tracking while holding
    # the lock. env.cleanup() stays OUTSIDE — Modal/Docker teardown can block 10-15s.
    ...
    # Replaced envs are no longer selectable: one-minute grace for concurrent
    # foreground calls, then force-removal once nothing references the scope.
...
def get_active_env(task_id: str, target: Optional[str] = None):
    ...
    When *target* names an execution target, its scoped cache keys are tried
    first (named targets register under ``resolution.environment_key(...)``);
    unnamed lookups keep the historical collapsed/raw fallback.
    """
    from tools.terminal_tool import (
        _active_environments, _env_lock, _resolve_container_task_id, _target_resolution,
    )
    ...
    if target:
        ...
        if resolution is not None and resolution.named:
            candidates += [
                resolution.environment_key(lookup),
                resolution.environment_key(task_id),
            ]
    candidates += [lookup, task_id]
    with _env_lock:
